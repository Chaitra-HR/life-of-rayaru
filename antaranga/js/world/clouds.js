...
export function createClouds({ count = 5, seed = 5, mobile = false } = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const mat = new THREE.SpriteMaterial({
      ...
      opacity: .15 + (1 - far) * .09,                           // thin veils — the dawn gradient stays legible through them
      color: new THREE.Color().setHSL(.72, .10, .84 - far * .04),   // dusty mauve, part of the dawn's own air
    });
    const s = new THREE.Sprite(mat);
    const w = 90 + far * 130 + rr() * 40;
    // long thin streaks lying low over the horizon — the dawn's striations,
    // never banks that wash out the upper sky's blue
    s.scale.set(w, w * (.055 + rr() * .03), 1);
    const z = -150 - far * 260;
    s.position.set((rr() - .5) * 300, 9 + far * 14 + rr() * 6, z);
    s.renderOrder = 1;
    g.add(s);
